...
이 프로젝트의 핵심은 PyTorch나 TensorFlow와 같이 **동적 연산 그래프(Dynamic Computational Graph)**를 기반으로 동작하는 딥러닝 추론 및 학습 엔진을 밑바닥부터 C++와 CUDA로 구현하는 것입니다.

## 핵심 개념

개념: 동적 연산 그래프 (Dynamic Computational Graph)

딥러닝의 학습은 거대한 합성함수의 미분 과정입니다. 시스템은 연산이 발생할 때마다 그 연산의 입력값과 결과값, 그리고 어떤 수학 연산이 수행되었는지를 메모리상에 그래프 형태로 기록합니다.

    노드 (Node): 데이터 버퍼인 **텐서(Tensor)**가 노드가 됩니다.

    간선 (Edge): 텐서와 텐서 사이의 수학적 변환을 수행하는 **함수(Function)**가 간선이 됩니다.

개념: 순전파 (Forward Pass)

입력 데이터가 신경망의 계층을 통과하며 최종 예측값(출력)을 만들어내는 과정입니다.
순전파가 진행되는 동안, 시스템은 단순히 수학 계산만 하는 것이 아니라 역전파에 필요한 데이터(Context)를 메모리에 저장하고, 텐서들을 grad_fn 포인터로 연결하여 연산 그래프를 구축합니다.
개념: 역전파 (Backward Pass)

최종 결과값(손실 함수 결과)에서부터 시작하여, 순전파 때 만들어둔 연산 그래프를 역순으로 타고 내려오며 편미분(Gradient)을 계산하는 과정입니다. 연쇄 법칙(Chain Rule)에 의해 각 연산의 도함수가 곱해지며, 최종적으로 모델 내부의 가중치 텐서들에 미분값이 누적됩니다.

## 엔진 작동 원리

1. 데이터 입력 및 순전파 연산
    입력 텐서가 Linear 계층(Layer)에 전달되면, 계층 내부에서 forward()가 호출됩니다.
    행렬곱 연산을 수행하는 MatmulFunction 인스턴스가 생성되며 forward()를 실행합니다.
    이때, 역전파에서 행렬 미분을 계산하려면 순전파의 입력값이 필요합니다. MatmulFunction은 입력 텐서들을 Context 객체에 담아 보관합니다. (save_for_backward() 사용)
    연산 결과로 새로운 출력 텐서가 생성됩니다. 이 출력 텐서의 grad_fn 변수에는 방금 자신을 만들어낸 MatmulFunction의 주소값이 저장됩니다.
2. 손실(Loss) 계산
    모델의 최종 출력 텐서와 정답 텐서를 비교하여 오차를 계산합니다.
    이 오차 값 역시 텐서이며, 연산 그래프의 가장 마지막 노드(Root)가 됩니다.
3. 역전파 연산 시작 (backward())
    오차 텐서에서 backward()를 호출합니다.
    엔진은 텐서의 grad_fn 포인터를 확인하여 직전에 수행된 Function을 찾고, 해당 함수 내부의 backward()를 호출합니다.
    Function의 backward()는 자신이 보관하고 있던 Context에서 순전파 당시의 입력 데이터를 꺼냅니다. (get_saved_tensors() 사용)
    미분 공식을 적용하여 계산된 기울기(Gradient) 텐서를 다시 이전 노드(입력 텐서)로 전달합니다.
    이 과정이 그래프의 끝점인 가중치(Weight) 텐서에 도달할 때까지 반복되며, 최종 도달한 가중치 텐서의 grad 메모리 공간에 미분값이 누적됩니다.
4. 매개변수 갱신 (Parameter Update)
    역전파가 끝나면 모델 내부의 모든 가중치 텐서에 기울기(grad)가 계산되어 있습니다.
    최적화 알고리즘(Optimizer)이 가중치 텐서들을 순회하며 실제 데이터를 갱신합니다. (수식: W=W−α⋅∇W)
    다음 학습을 위해 모든 텐서의 기울기를 0으로 초기화합니다. (zero_grad() 호출)
